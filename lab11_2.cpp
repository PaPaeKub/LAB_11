...
int calScore(int x,int y,int z){
 int score = cardScores[x] + cardScores[y] + cardScores[z];
    return score % 10;//Write the function to calculate the score with is the rightmost digit of summation of the scores from three cards.
 //Return the score. 
}
...
int findYugiAction(int s){ 
 if(s == 9) return 2; // Yugi will definitely stay (2) when current score (s) is equal to 9
  ...
  int r = rand() % 100;
        if(r < 69) return 1;
        else return 2;// If current score is 6,7,8, Yugi will draw with probability 69% and will stay with probability 31% 
...
int main(){ 
 ...
 cout << "Yugi's cards: " <<cardNames[yugiCards[0]]<<" "
    <<cardNames[yugiCards[1]]<< "\n";
